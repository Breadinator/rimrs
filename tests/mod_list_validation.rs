...
fn generate_mod_meta_data() -> Arc<Mutex<HashMap<String, ModMetaData>>> {
    ...
    mmd.insert(String::from("a"), ModMetaData {
        loadBefore: Some(HashSet::from_iter(vec![String::from("c")])),
        ..Default::default()
    });
    mmd.insert(String::from("b"), ModMetaData {
        modDependencies: Some(HashSet::from_iter(vec![Dependency {packageId: Some(String::from("a")), ..Default::default()}])),
        ..Default::default()
    });
    mmd.insert(String::from("c"), ModMetaData::default());
    mmd.insert(String::from("d"), ModMetaData {
        loadAfter: Some(HashSet::from_iter(vec![String::from("c")])),
        ..Default::default()
    });
    mmd.insert(String::from("e"), ModMetaData {
...
fn no_mod_metadata() {
    ...
    assert!(res.is_warn());
    assert_eq!(res.warnings().unwrap(), &vec![String::from("Couldn't find metadata for z")]);
}

#[test]
fn all_in_order() {
    let res = validate!(&[String::from("a"), String::from("b"), String::from("c"), String::from("d")]);
    assert!(res.is_ok());
}
...
fn load_after_wrong_order() {
    ...
    assert!(res.is_warn());
    assert_eq!(res.warnings().unwrap(), &vec![String::from("d should be loaded after c")]);
}
...
fn load_before_wrong_order() {
    ...
    assert!(res.is_warn());
    assert_eq!(res.warnings().unwrap(), &vec![String::from("a should be loaded before c")]);
}
...
fn incompatible() {
    ...
    assert!(res.is_err());
    assert_eq!(res.errors().unwrap(), &vec![String::from("b requires a"), String::from("e is incompatible with b")]);
}
