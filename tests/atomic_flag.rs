...
fn set_reset_threaded() {
    let handles: Vec<_> = (0..100).map(|i| {
        thread::Builder::new().name(format!("{i}")).spawn(move || {
...
fn less_contrived() {
    ...
    let setters: Vec<_> = (0..100).map(|i| {
        thread::Builder::new().name(format!("less_contrived::setter{i}")).spawn(move || {
            for _ in 0..100 {
                flag.set();
                yield_now();
    ...
    let getter = thread::Builder::new().name(String::from("less_contrived::getter")).spawn(move || {
        loop {
            match rc.try_recv() {
                Ok(_) => return,
